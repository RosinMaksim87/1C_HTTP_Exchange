...
&НаКлиенте
Процедура ВыполнитьВыгрузкуИОтправитьПоHTTP(Команда) // 11.06.2026 Росин. Выгрузка в XML и отправка по HTTP
	ВыполнитьВыгрузкуИзФормы(Истина);
КонецПроцедуры
...
Процедура ВыполнитьЗагрузкуНаСервере(АдресФайла, ИмяФайлаДляРасширения)
	...
	Элементы.ВыполнитьВыгрузкуИОтправитьПоHTTP.Доступность = Ложь;
...
&НаКлиенте
Процедура ВыполнитьВыгрузкуИзФормы(ОтправитьПоHTTP = Ложь)
	...
	// 15.06.2026 Росин. Отправляем данные сразу по HTTP при необходимости
	Результат = ВыполнитьВыгрузкуНаСервере(ОтправитьПоHTTP);
	
	Если Результат = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если ОтправитьПоHTTP Тогда
		// 15.06.2026 Росин. Обработка результата отправки данных по HTTP
		Если ЗначениеЗаполнено(Результат.СтруктураОтветаHTTP.errMessage) Тогда
			ТекстСообщения = "Ошибка выполнения:"
							+ ?(ЗначениеЗаполнено(Результат.СтруктураОтветаHTTP.errCode), " Код ошибки " + Результат.СтруктураОтветаHTTP.errCode, "") + Символы.ПС
							+ Результат.СтруктураОтветаHTTP.errMessage;
			
			ПоказатьПредупреждение(,ТекстСообщения);
		Иначе
			ТекстСообщения = Результат.СтруктураОтветаHTTP.Сообщение;
		КонецЕсли;
	...
	Иначе
		...
		Если ЭтоКлиент И Не ПрямаяВыгрузка И Не Объект.ФлагОшибки Тогда
			...
			ПолучитьФайл(Результат.АдресФайлаДанных, ИмяСохраняемогоФайла)
		КонецЕсли;
...
&НаСервере
Функция ВыполнитьВыгрузкуНаСервере(ОтправитьПоHTTP)
	Объект.ИмяФайлаПравилОбмена = ИмяФайлаНаСервереИлиКлиенте(ИмяФайлаПравил, АдресФайлаПравилВХранилище);
	...
	Если ЭтоКлиент И Не ПрямаяВыгрузка Тогда
		Если ОтправитьПоHTTP Тогда
			// 15.06.2026 Росин
			ФайлНаДиске = Новый Файл(Объект.ИмяФайлаОбмена);
			Если Не ФайлНаДиске.Существует() Тогда
				Возврат Неопределено
			КонецЕсли;
			
			ДвоичныеДанныеФайла = Новый ДвоичныеДанные(Объект.ИмяФайлаОбмена);
			
			СтруктураОтветаHTTP = ОтправитьФайлПоHTTPНаСервере(ДвоичныеДанныеФайла);
	...
	Возврат Новый Структура("АдресФайлаДанных, СтруктураОтветаHTTP", АдресФайлаДанных, СтруктураОтветаHTTP); 
КонецФункции
...
Процедура УстановитьПризнакЗагрузкиПравил(Признак)
	...
	Элементы.ВыполнитьВыгрузкуИОтправитьПоHTTP.Доступность = Признак;
...
&НаСервере
Функция ОтправитьФайлПоHTTPНаСервере(ДвоичныеДанныеФайла) // 11.06.2026 Росин
	РезультатВыполнения = Новый Структура("errCode, errMessage, Сообщение", 0, "", "");
	
	ПараметрыПодключения = оо_ФункцииДляРаботыСНТТР.ПолучитьНастройкиПодключения("оо_HTTPОбмен", Метаданные.Имя);
	...
	//ИмяСервера		= ПараметрыПодключения.ИмяСервера;
	//Порт			= 80;
	//Пользователь	= "http_exchange";
	//Пароль			= "";
	
	Прокси = ПолучениеФайловИзИнтернета.ПолучитьПрокси("http://" + ИмяСервера);
	
	// Для упаковки файла в JSON надо перегнать двоичные данные в Base64
	СтрокаПараметровBase64 = Base64Строка(ДвоичныеДанныеФайла);
	СтрокаПараметровBase64 = СтрЗаменить(СтрокаПараметровBase64, Символы.ВК + Символы.ПС, ""); // Убираем переносы
	
	ПараметрыДляОтправки = Новый Структура("uid, useruid, from, fileData"
											, Строка(Новый УникальныйИдентификатор)
											, Строка(ПараметрыСеанса.ТекущийПользователь.ИдентификаторПользователяИБ)
											, Метаданные.Имя
											, СтрокаПараметровBase64);
	
	Попытка
		СтрокаJSON = ОбщегоНазначения.ЗначениеВJSON(ПараметрыДляОтправки);
	Исключение
		РезультатВыполнения.errMessage = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗаписьЖурналаРегистрации("ОбменДаннымиПоHTTP", УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		
		Возврат РезультатВыполнения;
	КонецПопытки;
	
	Попытка
		Соединение = Новый HTTPСоединение(ИмяСервера, Порт, Пользователь, Пароль, Прокси, 120);
		
		ИмяРесурса = ПараметрыПодключения.ПрефиксПути + "/ru_RU/hs/oo_exchange/downloadData"; 
		ЗапросHTTP = Новый HTTPЗапрос(ИмяРесурса);
		
		// Передаем параметры в кастомный заголовок, а файл — в тело
		//ЗапросHTTP.Заголовки.Вставить("ExchangeParams", СтрокаПараметровBase64);
		ЗапросHTTP.Заголовки.Вставить("Content-Type", "application/json charset=utf-8"); // Тип самого файла
		...
		Если ОтветHTTP.КодСостояния = 200 Тогда
			СтрокаОтветаJSON = ОтветHTTP.ПолучитьТелоКакСтроку(КодировкаТекста.UTF8);
			
			ЧтениеJSON = Новый ЧтениеJSON;
			ЧтениеJSON.УстановитьСтроку(СтрокаОтветаJSON);
			РезультатВыполнения = ПрочитатьJSON(ЧтениеJSON);
		Иначе
			РезультатВыполнения.errMessage = "Ошибка в результате при отправке HTTP запроса.";
			РезультатВыполнения.errCode = ОтветHTTP.КодСостояния;
		КонецЕсли;
	Исключение
		РезультатВыполнения.errMessage = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗаписьЖурналаРегистрации("ОбменДаннымиПоHTTP", УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
