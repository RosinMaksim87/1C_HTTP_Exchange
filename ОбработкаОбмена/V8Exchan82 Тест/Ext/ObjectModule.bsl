...
Процедура ОткрытьДанныеПротоколовОбменаПриНеобходимости()
	...
	Если Не ПустаяСтрока(ИмяФайлаПротоколаОбмена) Тогда
		ОткрытьВПриложении(ИмяФайлаПротоколаОбмена);
	КонецЕсли;
	...
	Если НепосредственноеЧтениеВИБПриемнике Тогда
		...
		Если Не ПустаяСтрока(ИмяПротоколаЗагрузки) Тогда
			ОткрытьВПриложении(ИмяПротоколаЗагрузки);
		КонецЕсли;
...
//  
// 
Процедура ОткрытьВПриложении(ИмяФайла)
...
//
Функция ЭтоПлатформа83БезРежимаСовместимости()
